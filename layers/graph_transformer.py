...
def make_model(d_atom, N=2, d_model=128, h=8, dropout=0.1,
               ...
               scale_norm=False, init_type='uniform', n_generator_layers=1):  # 移除use_adapter参数
    c = copy.deepcopy
    attn = MultiHeadedAttention(h, d_model, dropout, distance_matrix_kernel, use_edge_features, control_edges,
                                integrated_distances)
    ff = PositionwiseFeedForward(d_model, N_dense, dropout, leaky_relu_slope, dense_output_nonlinearity)
    model = GraphTransformer(
        Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout, scale_norm), N, scale_norm),  # 移除use_adapter
...
class SublayerConnection(nn.Module):
    def __init__(self, size, dropout, scale_norm):  # 移除use_adapter参数
    ...
    def forward(self, x, sublayer):
        # 移除Adapter相关逻辑，仅保留基础残差连接
        return x + self.dropout(sublayer(self.norm(x)))


class EncoderLayer(nn.Module):
    def __init__(self, size, self_attn, feed_forward, dropout, scale_norm):  # 移除use_adapter参数
        super(EncoderLayer, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        # 移除use_adapter传递，仅初始化2个子层连接
...
def attention(query, key, value, adj_matrix, distances_matrix, edges_att,
    ...
    p_weighted = p_attn + p_dist + p_adj  # 系数均为1的求和
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1,
        ...
        if use_edge_features:
            d_edge = 9 if not integrated_distances else 10
